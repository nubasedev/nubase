...
As an example: the "examples/questlog" folder contains 3 projects

examples/questlog/backend
examples/questlog/frontend
examples/questlog/schema

The schema is defined in examples/questlog/schema and both the frontend and backend use this common schema to configure itself.
...
**1. examples/questlog (Monorepo Example)**
...
- `examples/questlog/frontend` - React frontend example using Vite that demonstrates Nubase in action. It runs in tavern.localhost:3002
- `examples/questlog/backend` - Node.js backend example with API endpoints. It runs in tavern.localhost:3001
- `examples/questlog/schema` - Shared schema definitions for the example app

**Use this when**: Contributing to Nubase, testing local package changes, or understanding the framework internals.

**2. examples/generated (Standalone Example)**

This is a standalone application generated by `npx @nubase/create`. It uses published `@nubase/*` packages from npm and has its own Turborepo configuration. This folder is git-ignored since it's meant to be regenerated for testing.
...
**Creating and testing the generated example:**

```bash
# Create using local @nubase/create (for testing local changes)
npm run examples:generated:create:local

# Create using npm (tests the published CLI)
npm run examples:generated:create:npm

# Run full E2E tests on the generated example
npm run examples:generated:test
```

### Example Application Architecture

When you run the development environment (`npm run dev`), you're running the questlog example application located in the `examples/questlog/` folder. This demonstrates a real-world Nubase application with three interconnected parts:

**Frontend (`examples/questlog/frontend`)**

- The main application configuration is defined in `examples/questlog/frontend/src/config/config.ts`
- The application entry point (`examples/questlog/frontend/src/main.tsx`) simply renders the `NubaseApp` component with this configuration
...
5. **IMPORTANT**: Update `examples/questlog/frontend/src/config/config.ts` to include the new theme ID in the `themeIds` array - themes won't be visible in the example application without this step
...
```bash
# 1. Create the generated example using local @nubase/create
npm run examples:generated:create:local

# 2. Run E2E tests on the generated example
npm run examples:generated:test
```
