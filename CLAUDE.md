...
As an example: the "questlog-example-app" folder contains 3 projects

questlog-example-app/backend
questlog-example-app/frontend
questlog-example-app/schema

The schema is defined in questlog-example-app/schema and both the frontend and backend use this common schema to configure itself.
...
- `questlog-example-app/frontend` - React frontend example using Vite that demonstrates Nubase in action. It runs in tavern.localhost:3002
- `questlog-example-app/backend` - Node.js backend example with API endpoints. It runs in tavern.localhost:3001
- `questlog-example-app/schema` - Shared schema definitions for the example app

### Example Application Architecture

When you run the development environment (`npm run dev`), you're running the example application located in the `questlog-example-app/` folder. This demonstrates a real-world Nubase application with three interconnected parts:

**Frontend (`questlog-example-app/frontend`)**

- The main application configuration is defined in `questlog-example-app/frontend/src/config/config.ts`
- The application entry point (`questlog-example-app/frontend/src/main.tsx`) simply renders the `NubaseApp` component with this configuration
...
5. **IMPORTANT**: Update `questlog-example-app/frontend/src/config/config.ts` to include the new theme ID in the `themeIds` array - themes won't be visible in the example application without this step
