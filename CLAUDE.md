...
As an example: the "examples/internal" folder contains 3 projects

examples/internal/backend
examples/internal/frontend
examples/internal/schema

The schema is defined in examples/internal/schema and both the frontend and backend use this common schema to configure itself.
...
- `examples/internal/frontend` - React frontend example using Vite that demonstrates Nubase in action. It runs in tavern.localhost:3002
- `examples/internal/backend` - Node.js backend example with API endpoints. It runs in tavern.localhost:3001
- `examples/internal/schema` - Shared schema definitions for the example app

### Example Application Architecture

When you run the development environment (`npm run dev`), you're running the example application located in the `examples/internal/` folder. This demonstrates a real-world Nubase application with three interconnected parts:

**Frontend (`examples/internal/frontend`)**

- The main application configuration is defined in `examples/internal/frontend/src/config/config.ts`
- The application entry point (`examples/internal/frontend/src/main.tsx`) simply renders the `NubaseApp` component with this configuration
...
5. **IMPORTANT**: Update `examples/internal/frontend/src/config/config.ts` to include the new theme ID in the `themeIds` array - themes won't be visible in the example application without this step
