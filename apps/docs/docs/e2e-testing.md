...
## Testing Both Example Applications

Nubase has two example applications, and both can be tested:

### examples/questlog (Monorepo Example)

The primary example application used for framework development. It's part of the Turborepo workspace and uses local `@nubase/*` packages.

```bash
# Run E2E tests on questlog example
npm run e2e
npm run e2e:ui      # Interactive mode
npm run e2e:headed  # With visible browser
```

### examples/generated (Standalone Example)

A standalone application generated by `npx @nubase/create`. It uses published `@nubase/*` packages from npm.

```bash
# Create and test the generated example
npm run examples:generated:create:local  # Or :create:npm
npm run examples:generated:test          # Runs install, typecheck, and E2E tests
```

The `examples:generated:test` script runs:
1. `npm install` - Install dependencies
2. `npm run typecheck` - Verify TypeScript
3. `npm run db:test:up` - Start test database
4. `npm run db:test:reset` - Reset database
5. `npm run e2e:install` - Install Playwright
6. `npm run e2e` - Run E2E tests

This ensures that the CLI-generated apps work correctly with published packages.
