...
- `examples/questlog/frontend/.env.local` - Local development overrides
- `examples/questlog/backend/.env` - Backend configuration
...
Tests are located in `examples/questlog/frontend/e2e/` and follow this pattern:
...
The test configuration is located in `examples/questlog/frontend/playwright.config.ts`:
...
#### `examples/questlog/frontend/.env.local` (Optional overrides)
```bash
