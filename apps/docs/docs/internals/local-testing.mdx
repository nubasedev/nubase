...
3. Runs the built CLI to scaffold `apps/starter/` from the templates
4. Runs `npm install` at the workspace root so `apps/starter` links to the local `@nubase/*` packages

Because `apps/*` is in the root `workspaces` array, `apps/starter` becomes a workspace member: its `@nubase/*` deps resolve to the local sources via symlinks. No npm publish required to test the templates end-to-end.
