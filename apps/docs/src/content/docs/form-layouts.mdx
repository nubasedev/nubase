...
const contactSchema = nu.object({
  ...
  email: nu.string().email().withMeta({ label: 'Email' }),
  phone: nu.string().withMeta({ label: 'Phone' }),
  company: nu.string().withMeta({ label: 'Company' }),
  notes: nu.string().withMeta({ label: 'Notes' }),
}).withLayout({
  type: 'form',
  groups: [
    {
...
const schema = nu.object({
  email: nu.string().email().withMeta({
    label: 'Email',
    description: 'Enter a valid email address',
  }),
  age: nu.number().min(18).max(120).withMeta({
    label: 'Age',
