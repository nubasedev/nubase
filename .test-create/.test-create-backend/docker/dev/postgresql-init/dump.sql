-- .testCreate Database Schema
-- This file is the source of truth for database structure

-- Create app user with restricted permissions (for RLS)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '.test_create_app') THEN
        CREATE ROLE .test_create_app WITH LOGIN PASSWORD '.test_create';
    END IF;
END
$$;

-- Enable RLS on the database
ALTER DATABASE .test_create SET row_security = on;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON workspaces TO .test_create_app;
GRANT USAGE, SELECT ON SEQUENCE workspaces_id_seq TO .test_create_app;
...
-- Grant permissions to app user
GRANT SELECT, INSERT, UPDATE, DELETE ON users TO .test_create_app;
GRANT USAGE, SELECT ON SEQUENCE users_id_seq TO .test_create_app;
...
-- Grant permissions to app user
GRANT SELECT, INSERT, UPDATE, DELETE ON user_workspaces TO .test_create_app;
...
-- Grant permissions to app user
GRANT SELECT, INSERT, UPDATE, DELETE ON tickets TO .test_create_app;
GRANT USAGE, SELECT ON SEQUENCE tickets_id_seq TO .test_create_app;
