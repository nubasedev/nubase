// @ts-nocheck — vendored code, see header below.
// Vendored from https://github.com/adelsz/pgtyped (MIT).
// Original: packages/query/src/actions.ts
// See packages/pg/LICENSE-pgtyped.md for the license and adaptations.

import crypto from 'crypto';
import * as tls from 'tls';
import { AsyncQueue } from '../wire/queue.js';
import { messages, PreparedObjectType } from '../wire/messages.js';
import {
...
// Adaptation: pgtyped originally imports `InterpolatedQuery` and
// `QueryParameter` from `@pgtyped/runtime`. We only need the minimal structural
// shape — `@nubase/pg` does its own SQL preprocessing — so we define loose
// local types here instead of pulling in the runtime package.
...
// Adaptation: pgtyped uses the `debug` package; we replace it with a no-op
// so `@nubase/pg` has no dependency on `debug`.
const debugQuery = (_msg: string, ..._args: unknown[]): void => {};
...
export async function startup(
  ...
  try {
    ...
    // Adaptation: pgtyped calls `process.exit(1)` here. Library code must not
    // terminate the host process — we wrap and rethrow so the caller can
    // decide how to handle it.
    const msg = e instanceof Error ? e.message : String(e);
    throw new Error(`Postgres wire connection failed: ${msg}`);
  }
}
