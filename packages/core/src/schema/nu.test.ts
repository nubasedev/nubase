...
describe('nubase Schema Library (nu)', () => {
  ...
  describe('Computed Metadata', () => {
    ...
    it('should get all merged metadata for an object schema', async () => {
      ...
      expect(mergedMeta.name.description).toBe('Computed description for Test Product');
    ...
    it('should handle partial form data gracefully in merged metadata', async () => {
      const productSchema = nu.object({
        ...
        name: {
          ...
          description: async (obj) => `Product in ${obj.category} category, priced at $${obj.price}`,
        }
      });
      ...
      expect(mergedMeta.name.description).toBe('Product in General category, priced at $0');
    });
