...
> & {
  ...
  if (
    ...
    const summaryRow = topSummaryRows[rowIdx + topSummaryRowsCount];
    if (summaryRow) {
      return getColSpan(column as any, lastFrozenColumnIndex, {
        type: "SUMMARY",
        row: summaryRow,
      });
    }
  ...
  if (bottomSummaryRows) {
    const summaryRow = bottomSummaryRows[rowIdx - rows.length];
    if (summaryRow) {
      return getColSpan(column as any, lastFrozenColumnIndex, {
        type: "SUMMARY",
        row: summaryRow,
...
export function getNextSelectedCellPosition<R, SR>({
  ...
  const setHeaderGroupColAndRowSpan = () => {
    if (moveNext) {
      ...
      if (!nextColumn) return { idx: nextIdx, rowIdx: nextRowIdx };
