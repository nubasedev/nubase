...
export function DataGrid<R, SR = unknown, K extends Key = Key>(
  ...
  const headerRowHeight =
    rawHeaderRowHeight ?? (typeof rowHeight === "number" ? rowHeight : 35);
  ...
  const renderCell =
    renderers?.renderCell ?? defaultRenderers?.renderCell ?? defaultRenderCell;
  ...
  const [draggedOverRowIdx, setDraggedOverRowIdx] = useState<
    number | undefined
  >(undefined);
  ...
  const isColumnWidthsControlled =
    columnWidthsRaw != null &&
    onColumnWidthsChangeRaw != null &&
    !isColumnResizing;
  const columnWidths = isColumnWidthsControlled
    ? columnWidthsRaw
    : columnWidthsInternal;
  ...
  const [gridRef, gridWidth, gridHeight, horizontalScrollbarHeight] =
    useGridDimensions();
  const {
    columns,
    colSpanColumns,
  ...
  function selectRow(args: SelectRowEvent<R>) {
    ...
    if (
      ...
      for (let i = previousRowIdx + step; i !== rowIdx; i += step) {
        const row = rows[i];
        if (!row || isRowSelectionDisabled?.(row) === true) continue;
        if (checked) {
  ...
  function commitEditorChanges() {
    ...
    if (!column) return;
    updateRow(column, selectedPosition.rowIdx, selectedPosition.row);
  }
  ...
  function handleDragHandlePointerMove(
    ...
    const headerAndTopSummaryRowsHeight =
      headerRowsHeight + topSummaryRowsCount * summaryRowHeight;
    const offset =
      scrollTop -
  ...
  function updateRows(startRowIdx: number, endRowIdx: number) {
    ...
    if (!column || !sourceRow) return;
    const updatedRows = [...rows];
    const indexes: number[] = [];
    for (let i = startRowIdx; i < endRowIdx; i++) {
      if (isCellEditable({ rowIdx: i, idx })) {
        const targetRow = safeGet(rows, i);
        ...
        if (updatedRow && updatedRow !== targetRow) {
    ...
    if (onSelectedCellChange && !samePosition) {
      ...
      if (column) {
        onSelectedCellChange({
          ...
          row: isRowIdxWithinViewportBounds(position.rowIdx)
            ? rows[position.rowIdx]
            : undefined,
          column,
        });
      }
  ...
  function getDragHandle() {
    ...
    const { idx, rowIdx } = selectedPosition;
    const column = columns[idx];
    if (!column || column.renderEditCell == null || column.editable === false) {
      return;
    }
    ...
    const row = rows[rowIdx];
    if (!row) return;
    const colSpan = column.colSpan?.({ type: "ROW", row }) ?? 1;
    ...
    return (
      <div
        ...
        onLostPointerCapture={
          isDragging ? handleDragHandleLostPointerCapture : undefined
        }
  ...
  function getCellEditor(rowIdx: number) {
    ...
    const closeOnExternalRowChange =
      column.editorOptions?.closeOnExternalRowChange ?? true;
...
// Utility to safely access array elements with bounds checking
function safeGet<T>(arr: readonly T[], index: number): T | undefined {
  return index >= 0 && index < arr.length ? arr[index] : undefined;
}

// Export the Column type as DataGridColumnDef for compatibility
export type { Column as DataGridColumnDef } from "./types";
